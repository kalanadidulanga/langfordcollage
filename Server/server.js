...
async function addNewUni(req, res) {
    const {title, description, banner_path, image_path, link} = req.body;

    try {
        const [results] = await db.query(
            "SELECT * FROM university WHERE title=?", [
                title
            ]
        );
        if (results.length > 0) {
            return res
                .status(400)
                .json({error: "University Already Exists, Please Try Again."});
        }

        const sql = `
            INSERT INTO blog (title, description, image_path, banner_path, link)
            VALUES (?, ?, ?, ?, ?)
        `;
...
async function enrollNow(req, res) {
    ...
    const sql = `
        INSERT INTO course_enroll_request (fname, lname, email, phone, course, course_level, study_qualification,
                                           job_title, payment_option)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const values = [
        first_name,
...
async function acceptEnroll(req, res) {
    ...
    const sql = `
        INSERT INTO enrolled_students (fname, lname, email, phone, course_level, course,
